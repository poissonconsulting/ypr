...
test_that("ypr_update population errors if multiple values", {
  expect_error(
    ypr_update(ypr_population(), Rk = c(2.5, 1.5)),
...
test_that("ypr_update population errors if missing value", {
  expect_error(
    ypr_update(ypr_population(), Rk = NA_real_),
...
test_that("ypr_update population errors if outside range", {
  expect_error(
    ypr_update(ypr_population(), Rk = 1001),
...
test_that("ypr_update populations renames", {
  expect_named(
    ypr_update(ypr_populations(Rk = c(2.5, 1.5), names = c("t1", "t2")), Linf = 200),
    c("Rk_1_5", "Rk_2_5")
  )
})
...
test_that("ypr_update ecotypes renames", {
  expect_named(
    ypr_update(ypr_ecotypes(k = c(2.5, 1.5), names = c("t1", "t2")), Linf = 200),
    c("k_2_5", "k_1_5")
  )
})
...
test_that("ypr_update ecotypes errors if end up with multiple the same", {
  expect_error(
    ypr_update(ypr_ecotypes(Rk = c(2.5, 1.5)), Rk = 2.5),
