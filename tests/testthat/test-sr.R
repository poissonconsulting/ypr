...
test_that("sr", {
  ...
  expect_is(bh, "list")
  expect_identical(names(bh), c("alpha", "beta", "phi", "phiF", "R0", "R0F", "S0", "S0F"))
  ...
  expect_equal(bh$S0, 0.7919029, check.attributes = FALSE, tolerance = 1e-06)
  expect_equal(bh$S0F, 0.08733441, check.attributes = FALSE, tolerance = 1e-06)
  ...
  expect_identical(ypr_sr(ypr_population(Rk = bh$alpha)), bh)
  expect_identical(ypr_sr(ypr_population(BH = 0L, Rk = ri$alpha)), ri)
})
