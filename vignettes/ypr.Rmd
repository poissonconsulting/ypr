...
This requires the number of spawners per spawner at low density ($R_k$) and the recruitment age ($R_t$; by default 1) to be defined as well as the form of the stock-recruitment curve.
...
where $N_a$ is the annual interval natural mortality at age $a$ and the fished survivorship ($lx_a^F$) by
...
The value of $\beta$ for the Beverton-Holt curve can be found by rearranging the formula as follows
...
The number of recruits at the fished equilibrium ($R_{0F}$) can then be found for the Ricker curve as follows
