...
ypr_plot_fish(ecotypes, x = "Length", y = "Spawners", color = "white", binwidth = 15) + scale_fill_manual(values = c("black", "blue"))
```

### Stock-Recruitment

```{r, fig.width=6, fig.height=4}
ypr_plot_sr(ecotypes)
ypr_tabulate_sr(ecotypes)
```

### Yield

```{r, fig.width=6, fig.height=4}
#ypr_tabulate_yield(ecotypes)
#ypr_plot_yield(ecotypes)
```
